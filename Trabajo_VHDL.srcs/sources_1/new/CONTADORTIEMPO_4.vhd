...
entity CONTADORTIEMPO_4 is
    Port (
        ...
        count_out_4 : out INTEGER range 0 to 35999 -- Salida del contador (0 a 35999)
    );
...
architecture Behavioral of CONTADORTIEMPO_4 is
    ...
    -- Contador de tiempo
    signal count_4 : INTEGER range 0 to 35999 := 0; -- Contador interno
...
begin
    ...
    begin
        if reset = '1' then
            count_4 <= 0;  -- Reinicia el contador
            ...
            if enable_100ms = '1' then      -- Solo cuenta cuando el strobe está activo
                if count_4 = 35999 then
                    count_4 <= 0;             -- Reinicia el contador al alcanzar 35999
                else
                    count_4 <= count_4 + 1;     -- Incrementa el contador
                end if;
            end if;
        end if;
    end process;
